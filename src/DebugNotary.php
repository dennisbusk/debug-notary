...
class DebugNotary {
    ...
    public static function reportingRoutes(): void {
        ...
        $middleware = config('debug-notary.reporting_middleware', [ 'web' ]);

        Route::group([ 'middleware' => $middleware ], function () use ( $prefix ) {
    ...
    public static function managementRoutes(): void {
        $prefix     = config('debug-notary.route_prefix', 'laravel-debug-notary');
        $middleware = config('debug-notary.middleware', [ 'web', 'auth' ]);
